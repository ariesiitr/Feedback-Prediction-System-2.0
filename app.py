...
stop_words = stop_nltk + list(punctuation) + ["..."] + ["phone", "mobile","lenovo","k8","product","note","amazon", "got", "n't","'s"] + ["...."] + [".."]
...
def clean_txtlemma(sent):
    ...
    new_words = [word for word in new_words if word not in stop_words and len(word) >= 2]
    res = ' '.join(new_words)
    return res
